...
use ast::*;
use common::{
  errors::{BytecodeGenerationError, BytecodeGenerationErrorMsg, CompilerError},
  util::FileIdType,
  Error, InstructionData, LongAddr, Opcode, ShortAddr, SourceLocation, Storage, TryIntoInstruction,
};
use ptr::SmartPtr;
use std::fmt::Debug;
...
impl<'p> BytecodeGenerator<'p> {
  ...
  pub fn generate(mut self, ast: Ast) -> Result<SmartPtr<Context>, Error> {
    ...
    if self.errors.is_empty() {
      ...
      Err(Error::Compiler(CompilerError::BytecodeGeneration(self.errors)))
    }
  }
