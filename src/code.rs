...
impl<'ctx, 'file> Parser<'ctx, 'file> {
            ...
            if self.scope_depth > 0 {
                if let Some(local) = self.locals.last_mut() {
                    ...
                    self.error(
                        var_pos,
                        String::from(
                            "expected local variable instead of global, this is a parser error",
                        ),
                    );
                }
            }
            ...
            self.wrap_fn(name.clone(), self.index - 1, |this| {
                ...
                if let Some(token) = this.current() {
                    if token != Token::RightParen {
                        loop {
                            if let Some(param) =
                                this.parse_variable(String::from("expected parameter name"))
                            {
                                airity += 1;
    ...
    fn wrap_fn<F: FnOnce(&mut Parser) -> Option<usize>>(
        &mut self,
        name: String,
        index: usize,
        f: F,
    ...
    fn rule_for(token: &Token) -> ParseRule<'ctx, 'file> {
        match token {
            ...
            Token::BangEqual => {
                ParseRule::new(None, Some(Parser::binary_expr), Precedence::Equality)
            }
            ...
            Token::Identifier(_) => {
                ParseRule::new(Some(Parser::make_variable), None, Precedence::None)
            }
