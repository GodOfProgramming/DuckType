...
pub mod stdlib;
pub(crate) mod value;
use common::{util::*, Instruction, LongAddr, Opcode, ShortAddr, Storage};
...
impl Vm {
  ...
  pub fn run_string(&mut self, source: impl AsRef<str>, module: UsertypeHandle<ModuleValue>) -> Result<Value, Error> {
    ...
    let opts = CompileOpts { optimize: self.opt };
    let ctx =
      code::compile_string(&mut self.cache, source, opts).map_err(|e| Error::from_common(e, &self.filemap, source.as_ref()))?;
...
impl Vm {
  ...
  fn exec_req(&mut self) -> ExecResult {
    ...
    if found_file.is_none() {
      let this_dir = std::env::current_dir().map_err(Error::from)?;
      found_file = try_to_find_file(&this_dir, &required_file, &mut attempts);
    }
      ...
      _ => {
        ...
        if let Some(value) = self.cache.get_lib(file_id) {
          ...
          let new_ctx = code::compile_file(&mut self.cache, file_id, source, opts)
            .map_err(|err| Error::from_common(err, &self.filemap, &source))?;
